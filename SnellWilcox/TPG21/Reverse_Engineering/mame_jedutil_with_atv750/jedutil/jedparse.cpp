...
#define ROW_LEN 84
	...
	/* now loop over groups of ROW_LEN fuses and output non-default groups */
	for (i = 0; i < data->numfuses; i += ROW_LEN)
	{
		int stroffs;
		int j;

		/* build up a string of ROW_LEN fuses */
		stroffs = snprintf(tempbuf, 256, "L%05d ", i);
		for (j = 0; j < ROW_LEN && i + j < data->numfuses; j++)
